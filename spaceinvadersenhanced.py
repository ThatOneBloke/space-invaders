...
def drawwindow(red, yellow, redbullet, yellowbullet, player1health, player2health):
    ...
    if player1health <= 0:
            screen.fill("yellow")
            wintext1 = font2.render("yellow has won!!!", True, "black")
            restarttext = font2.render("press r to restart and q to quit", True, "black")
            screen.blit(wintext1, (450, 250))
            screen.blit(restarttext, (450, 350))
            gamerun = False
            pygame.display.update()
    if player2health <= 0:
            screen.fill("red")
            wintext2 = font2.render("red has won!!!", True, "black")
...
def handlebullets(yellowbullet, redbullet, yellow, red):
    ...
    for r in redbullet:
        for y in yellowbullet:
            if r.colliderect(y):
                redbullet.remove(r)
                yellowbullet.remove(y)
                break
    for i in yellowbullet:
        i.x -= bulletvelocity
...
def main():
    global player1health, player2health, gamerun
